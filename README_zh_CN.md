...
服务器。全部 19 个集合，五个工具，除了 MCP SDK 之外没有任何依赖。
...
**五个，不是十九个。**一个集合配一个工具的服务器，会逼着模型在还不知道任何一个装了
什么之前，先从 19 个相似的名字里挑一个。在这里，集合是一个参数，需要时再去查目录。

五个工具都声明自己是只读的，而这不只是一句承诺：整个包里没有任何 `POST`、`PUT`
...
**除了 `oura_compare`，它不做分析。**没有相关性，没有趋势，没有异常检测 ——
而那恰恰是别的服务器安放自身价值的地方。
...
`oura_compare` 之所以存在，是因为它回应了这个反对意见，而不是无视它：数字和带一起给出。
其余的在这里还没有一种能经受同样模拟的方法，所以这里没有它们。相关性是下一个候选，
而它有更多误导人的方式 —— 共同的每周规律、共同的趋势，以及反复试好几个滞后期，
直到有一个越过线。
