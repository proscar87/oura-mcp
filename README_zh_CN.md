...
服务器。全部 19 个集合，六个工具，除了 MCP SDK 之外没有任何依赖。
...
| `oura_compare` | 两个时段的差异是否超出该指标自身的噪声 |
| `oura_relate` | 两个指标的逐日变化是否在偶然之外一起变动 |
| `oura_check` | 什么都不暴露的自检 |

**六个，不是十九个。**一个集合配一个工具的服务器，会逼着模型在还不知道任何一个装了
什么之前，先从 19 个相似的名字里挑一个。在这里，集合是一个参数，需要时再去查目录。

六个工具都声明自己是只读的，而这不只是一句承诺：整个包里没有任何 `POST`、`PUT`
...
### `oura_compare`：唯一的计算，连同它的方法

「我戒酒以后 HRV 上去了吗？」才是人们真正带来的问题，而递过去两个平均值会把它答错：
每天的指标会自己晃动，好的一晚往往跟着另一个好的一晚，所以教科书式的比较
**大约每三次就有一次把普通的噪声叫作变化**（实测：在这些指标通常具有的自相关下为
34–38%）。

`oura_compare` 接受一个 `metric` —— `collection.field`，比如
`daily_readiness.score`、`sleep.average_hrv` 或 `daily_activity.steps` —— 以及两个
时段，`a_start`/`a_end` 和 `b_start`/`b_end`。它返回两个平均值、差值，以及
**这个指标在那么长的时段里自己会晃动的幅度带**，这条带用**你自己之前的 120 天**
测出，并针对那种一天连着一天的依赖做了校正。然后给出三种结论之一：

- `outside_noise` —— 差值大于这条带。水平确实不同；但不说明为什么，也不说明会持续。
- `within_noise` —— 没有大于。**这不等于「没有变化」**：用那些天的数据，一个小于
  这条带的真实变化被漏掉的次数会多于被看出来的次数，响应会把这一点说出来。
- `cannot_tell` —— 某个时段里有数值的天数少于 7 天，或者用来测量这条带的历史
  少于 60 天，或者该指标根本没有变化（Oura 的示例数据就是这样）。不会去猜一条带。

方法是通过模拟选出来的，而不是在模拟之前定下的：用从你的历史估出的带和一个 t 临界值，
在试过的每一种自相关下，把噪声叫作变化的情况**最多大约每二十次一次**，测试套件也把它
守在这个水平。今天被排除在外，因为它还在累积。`sleep` 取每天最长的那段主睡眠，
响应会说明这条规则。做很多次比较、只留下越过线的那一次，就会碰巧找到一次越线 ——
响应也会说明这一点。

### `oura_relate`：相关性，去掉了它说谎的那些方式

「高强度训练的日子，会不会让我第二天早上的准备度下降？」把两列数据算个相关，
会以两种常见的方式把它答错：两个都在周末上升的指标，彼此毫无关系也会相关；
两个都在几个月里缓慢漂移的指标也一样。在互不相关的模拟指标上实测，普通的相关
在共享每周节律时 **41–93% 的情况下**把它们判为相关，在共享趋势时为 58%。

`oura_relate` 接受两个指标 `x` 和 `y`、一个区间 `start`/`end`，以及一个以天为单位的
`lag`（0–7）。它从每个指标里去掉每个星期几的惯常水平，只比较相邻两天之间的
**逐日变化**，按自相关校正配对的数量，然后返回相关系数及其 95% 区间，以及同样的
三种结论。模拟过的每一种零假设情形 —— 每周节律、随机游走、共同的趋势、30% 的天数
缺失 —— 都保持在 5% 左右（实测最高 5.2%）。

- **滞后期由你来选，只选一次。**Oura 把一晚的睡眠和第二天早上的准备度记在你醒来的
  那一天名下，所以某一天的活动在 `lag=1` 时才会遇上紧随其后的那段睡眠。`first_pair`
  会显示哪一天和哪一天配成了对，让错误的对齐一眼可见。试好几个滞后期就是做好几次
  检验；而且因为这个方法作用在变化上，某个滞后期上的真实关系也会在相邻的滞后期上
  以相反的方向出现 —— 响应会把这两点都说出来。
- **它需要大约三个月。**有效配对少于 20 对时，它回答 `cannot_tell`；天数里有缺口时，
  需要的时间更长。
- **它衡量的是共同变动，不是原因，也不是方向。**别的东西可能同时推动两者，
  响应每一次都会这样说明。
...
**除了 `oura_compare` 和 `oura_relate`，它不做分析。**没有趋势，没有异常检测，没有建议 ——
而那恰恰是别的服务器安放自身价值的地方。
...
`oura_compare` 和 `oura_relate` 之所以存在，是因为它们回应了这个反对意见，而不是无视它：
数字和带一起给出。趋势在这里还没有一种能经受同样模拟的方法 —— 在自相关的日子上拟合
一条斜率，是所有信号里最容易制造出来的 —— 所以这里没有它们。

其余的一切，你拿到的都是原始数据。分析应该放在能够引用方法的地方 —— 比如
