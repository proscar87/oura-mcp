...
서버로 만든 것입니다. 19개 컬렉션 전부, 다섯 개의 도구, MCP SDK 외에는 의존성이
없습니다.
...
브라우저에서 **claude.ai**나 **ChatGPT**를 쓰시나요? 이들은 URL로 연결하므로
서버가 공개된 어딘가에 있어야 합니다. 당신의 Cloudflare 계정에 Worker로
배포하세요. 무료이고, 10분 정도 걸리며, 오직 당신만의 것입니다.
[원격: claude.ai와 ChatGPT](#원격-claudeai와-chatgpt)를 참고하세요.
...
Cloudflare Worker로 만든 것으로, **당신이 당신의 계정에, 당신의 Oura 계정만을
위해 배포합니다**. 누구도 공유 인스턴스를 운영하지 않으므로, 당신의 토큰과
데이터는 다른 누구도 거치지 않습니다.

로컬 서버에는 필요 없는 세 가지가 더해집니다. 어떤 앱이 요청하는지와 결과가
어디로 가는지를 보여 주는 자체 동의 페이지, Oura 로그인 뒤에 그 계정이 당신이
설정한 계정인지 확인하는 검사(다른 계정은 모두 거부되고, 소유자가 설정되지
않았다면 아무도 들어오지 못합니다), 그리고 Oura의 일회용 refresh token을 한 번에
한 요청씩 갱신하는 단 하나의 장소입니다.

단계별 배포 방법과, 무엇이 검증되었고 무엇이 검증되지 않았는지는 정확히 여기에
있습니다: [ts/worker/README.md](ts/worker/README.md).
...
| `oura_today` | 어젯밤의 수면과 오늘의 준비도, 그리고 그 앞의 날들 |
| `oura_compare` | 두 기간의 차이가 그 지표 자체의 잡음보다 큰지 |
| `oura_check` | 아무것도 노출하지 않는 자가 점검 |

**열아홉 개가 아니라 다섯 개입니다.** 컬렉션마다 도구를 하나씩 두는 서버는, 각각이
무엇을 담고 있는지 알기도 전에 모델에게 비슷한 이름 19개 중 하나를 고르라고
강요합니다. 여기서는 컬렉션이 파라미터이고, 목록은 필요할 때 조회합니다.

다섯 모두 스스로를 읽기 전용이라고 선언하는데, 이것은 약속이 아닙니다. 패키지 어디에도
`POST`, `PUT`, `DELETE`가 없고, 계속 그렇게 유지되도록 소스를 읽는 테스트가
있습니다.

`oura_today`는 정확성이 아니라 편의를 위해 존재하는 유일한 도구입니다. "어젯밤
잘 잤나?"라는 질문에 답하려면 오늘의 기록 두 건과, 그것이 평소와 다른지 판단할
만큼의 이력이 필요한데, 이전에는 네 번의 왕복이자 도중에 멈출 네 번의
기회였습니다. **아무것도 계산하지 않습니다.** 평균도, 차이값도, "당신의 HRV가
12% 올랐습니다"도 없습니다. 날짜별 데이터는 가공되지 않은 채로 돌아오고, 비교는
방법을 인용할 수 있는 곳에서 이루어집니다. 9년치 실제 데이터를 놓고 보면 연속된
두 측정값 사이의 변화 네 번 중 세 번은 그 지표 자체의 정상적인 진폭 안에
들어가므로, 그 맥락 없는 백분율은 신호를 보고하는 것이 아니라 만들어내는
것입니다. 유일한 파라미터는 `days`이고, 1부터 30까지, 기본값은 7입니다.

### `oura_compare`: 유일한 계산, 그 방법과 함께

"술을 끊은 뒤로 내 HRV가 올랐나?"는 사람들이 실제로 가져오는 질문이고, 평균 두
개를 건네는 것은 거기에 틀리게 답합니다. 일별 지표는 저절로 흔들리고, 좋은 밤
다음에는 좋은 밤이 오기 쉬워서, 교과서식 비교는 **평범한 잡음을 세 번에 한 번
꼴로 변화라고 부릅니다** (측정값: 이런 지표들이 흔히 갖는 자기상관에서
34–38%).

`oura_compare`는 `metric` — `daily_readiness.score`, `sleep.average_hrv`,
`daily_activity.steps` 같은 `collection.field` — 과 두 기간, `a_start`/`a_end`와
`b_start`/`b_end`를 받습니다. 두 평균, 그 차이, 그리고 **그만한 길이의 기간에서
이 지표가 저절로 움직이는 폭**을 답으로 돌려주는데, 이 폭은 **당신 자신의 직전
120일**로 측정하고 그 날과 날 사이의 의존성을 보정한 것입니다. 그다음 세 가지
판정 중 하나를 내립니다.

- `outside_noise` — 차이가 폭보다 큽니다. 수준이 다릅니다. 왜인지는 말하지 않고,
  계속될 것이라고도 말하지 않습니다.
- `within_noise` — 그렇지 않습니다. **이것은 "변화 없음"이 아닙니다.** 그 날들로는
  폭보다 작은 실제 변화를 알아채는 경우보다 놓치는 경우가 더 많고, 응답이 그렇게
  말합니다.
- `cannot_tell` — 한 기간에 값이 있는 날이 7일 미만이거나, 폭을 측정할 이력이
  60일 미만이거나, 지표가 전혀 변하지 않은 경우입니다(Oura 샘플 데이터처럼).
  폭을 추측하지 않습니다.

방법은 시뮬레이션 이전이 아니라 시뮬레이션으로 골랐습니다. 당신의 이력에서
추정한 폭과 t 임계값을 쓰면, 시험한 모든 자기상관에서 잡음을 변화라고 부르는 일은
**많아야 대략 스무 번에 한 번**이고, 테스트 스위트가 그 수준을 지키게 합니다.
오늘은 아직 쌓이는 중이므로 제외합니다. `sleep`은 각 날의 가장 긴 주 수면을 쓰고,
응답이 그 규칙을 밝힙니다. 비교를 여러 번 해서 경계를 넘은 하나만 남기면 우연히
넘은 것을 찾게 됩니다. 응답은 그것도 말해 줍니다.
...
**`oura_compare` 너머로는 분석하지 않습니다.** 상관관계도, 추세도, 이상 탐지도 없습니다. 다른 서버들이
...
`oura_compare`가 존재하는 것은 그 반론을 무시하지 않고 거기에 답하기 때문입니다.
숫자와 함께 폭이 옵니다. 나머지는 같은 시뮬레이션을 견디는 방법이 아직 여기에
없으므로, 여기에 없습니다. 다음 후보는 상관관계인데, 오도할 길이 더 많습니다.
공유된 주간 패턴, 공유된 추세, 그리고 하나가 경계를 넘을 때까지 여러 시차를
시험하는 것.

그 밖의 모든 것은 가공되지 않은 채로 받습니다. 분석은 방법을 인용할 수 있는 곳에 있어야
...
**원격 Worker를 배포한다면**, 한 가지 차이만 빼고 똑같습니다. 서버가 당신의
기기 대신 *당신의* Cloudflare 계정에서 돌기 때문에, 당신의 Oura 토큰은 그곳의
Durable Object에 저장됩니다. 앱들이 Worker에 쓰는 토큰은 KV에 해시로 보관되며,
토큰에 담긴 내용은 암호화됩니다. 건강 데이터는 여전히 절대 저장되지 않습니다.
요청마다 가져와 Worker의 메모리에만 머뭅니다. 당신 말고는 누구도 그것을
운영하지 않습니다. 자세한 내용:
