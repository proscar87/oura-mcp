...
서버로 만든 것입니다. 19개 컬렉션 전부, 여섯 개의 도구, MCP SDK 외에는 의존성이
없습니다.
...
| `oura_relate` | 두 지표의 하루하루 변화가 우연 이상으로 함께 움직이는지 |
| `oura_check` | 아무것도 노출하지 않는 자가 점검 |

**열아홉 개가 아니라 여섯 개입니다.** 컬렉션마다 도구를 하나씩 두는 서버는, 각각이
무엇을 담고 있는지 알기도 전에 모델에게 비슷한 이름 19개 중 하나를 고르라고
강요합니다. 여기서는 컬렉션이 파라미터이고, 목록은 필요할 때 조회합니다.

여섯 모두 스스로를 읽기 전용이라고 선언하는데, 이것은 약속이 아닙니다. 패키지 어디에도
...
### `oura_relate`: 상관관계, 그것이 거짓말하는 방식들을 걷어내고

"힘들게 운동한 날은 다음 날 아침 준비도가 떨어질까?" 두 열의 상관관계를 구하는
것은 익숙한 두 가지 방식으로 여기에 틀리게 답합니다. 주말마다 함께 오르는 두
지표는 서로 아무 관련이 없어도 상관관계를 보이고, 몇 달에 걸쳐 함께 흘러가는 두
지표도 마찬가지입니다. 서로 무관한 시뮬레이션 지표로 측정해 보면, 단순한
상관관계는 주간 리듬을 공유할 때 **41–93%의 경우** 둘이 관련 있다고 불렀고, 추세를
공유할 때는 58%였습니다.

`oura_relate`는 두 지표 `x`와 `y`, 범위 `start`/`end`, 그리고 일 단위의 `lag`
(0–7)를 받습니다. 각 지표에서 요일별 평소 수준을 빼고, 연속된 날들 사이의 **하루
하루 변화**만 비교하며, 쌍의 수를 자기상관에 맞춰 보정한 뒤, 상관계수를 95%
구간과 같은 세 가지 판정과 함께 돌려줍니다. 시뮬레이션한 모든 귀무 상황 — 주간
리듬, 랜덤 워크, 공유된 추세, 30%의 날이 빠진 경우 — 이 약 5%에 머뭅니다(측정값:
최대 5.2%).

- **시차는 당신이 한 번 고릅니다.** Oura는 하룻밤의 수면과 다음 날 아침의 준비도를
  잠에서 깬 날 아래에 기록하므로, 어떤 날의 활동은 `lag=1`에서 그 뒤에 이어진
  수면과 만납니다. `first_pair`가 어느 날이 어느 날과 짝지어졌는지 보여 주므로
  잘못된 정렬이 눈에 보입니다. 여러 시차를 시험하는 것은 여러 번 검정하는
  것이고, 이 방법은 변화를 다루기 때문에 한 시차의 실제 관계는 그 옆 시차에서도
  반대 방향으로 나타납니다. 응답은 둘 다 말해 줍니다.
- **석 달쯤 필요합니다.** 유효 쌍이 20개 미만이면 `cannot_tell`로 답하고, 날짜에
  빈틈이 있으면 더 오래 필요합니다.
- **함께 움직이는 것을 측정할 뿐, 원인도 방향도 아닙니다.** 다른 무언가가 둘을
  함께 움직일 수 있고, 응답은 매번 그렇게 말합니다.
...
**`oura_compare`와 `oura_relate` 너머로는 분석하지 않습니다.** 추세도, 이상 탐지도, 조언도 없습니다. 다른 서버들이
...
`oura_compare`와 `oura_relate`가 존재하는 것은 그 반론을 무시하지 않고 거기에
답하기 때문입니다. 숫자와 함께 폭이 옵니다. 추세는 같은 시뮬레이션을 견디는
방법이 아직 여기에 없습니다. 자기상관이 있는 날들 위의 기울기야말로 가장 만들어
내기 쉬운 신호이기 때문입니다. 그래서 여기에 없습니다.
